...
interface Review {
    ...
    rating: number;
    comment: string;
    reviewerName: string; // We will fill this with real data now
    reviewerId: number;   // Added this to track ID
    date: string;
}
...
export default function GenericProfilePage() {
    ...
    useEffect(() => {
        const fetchProfileData = async () => {
            ...
            try {
                ...
                // 2. Fetch Reviews
                const reviewRes = await fetch(`http://localhost:8080/api/reviews/user/${userId}?role=${role}`, {
                    headers: { "Authorization": `Bearer ${token}` }
                });
                ...
                if (reviewRes.ok) {
                    ...
                    // --- NEW LOGIC: FETCH NAMES FOR REVIEWERS ---
                    
                    // A. Extract all reviewer IDs
                    ...
                    // B. If we have IDs, fetch their names from Auth Service
    ...
    const isViewingBuyer = role === "BUYER";

    return (
        <div className="min-h-screen bg-gray-50/50">
            {role === "SELLER" ? <SellerHeader /> : <BuyerHeader />}
                    ...
                    <Card className="lg:col-span-1 p-8 border-none shadow-xl bg-white rounded-[30px] h-fit">
                        <div className="flex flex-col items-center text-center">
                            ...
                            <div className="flex items-center gap-2 px-4 py-2 bg-[#F0FDF4] text-[#166534] rounded-full text-xs font-bold uppercase tracking-wider mb-8">
                                ...
                                {isViewingBuyer ? "Verified Buyer" : "Verified Seller"}
                            </div>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                            <Card className="p-6 border-none shadow-md bg-white rounded-3xl flex flex-col items-center justify-center text-center gap-2">
                                ...
                                <h3 className="text-3xl font-black text-[#03230F]">{averageRating}</h3>
                                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Avg Rating</p>
                            </Card>
                            <Card className="p-6 border-none shadow-md bg-white rounded-3xl flex flex-col items-center justify-center text-center gap-2">
                                <ShoppingBag className="w-8 h-8 text-blue-500" />
                                <h3 className="text-3xl font-black text-[#03230F]">{reviews.length}</h3>
